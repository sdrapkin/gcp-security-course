...
- **Service account quotas** are project-scoped (a default limit in the low hundreds per project as of recent documentation, raisable via support request) – relevant when designing patterns that mint one SA per microservice or per tenant at scale; at high tenant counts this nudges architectures toward Workload Identity Federation or per-tenant impersonation rather than literally one persistent SA object per tenant.
